...
How you choose to build your helm chart is up to you, you can put every api object in the `templates` directory or have subcharts for each individual microservice. It doesn't matter to the library. All it does is clone the github repository and deploy the chart using the specified values file.

For most users, the only branch of the helm config. repo. needed is the "master" branch. However, if you want to use different branches for different app environments, you can add a `helm_chart_branch` setting to your application environments in your pipeline config.
