...
If using the SDP Pipeline Libraries as a xref:jte:library-development:library_sources/library_sources.adoc[Library Source] for your pipeline, then you *must* include the ``sdp`` library.  This library contains helper functions such as ``node()`` to facilitate the ability to run the SDP pipeline steps on a variety of agents (such as kuberenetes jenkins agents launched on demand, docker agents or generic agents that could be running in containers or VMs). 
